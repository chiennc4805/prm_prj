...
class ClbDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      backgroundColor: Colors.grey.shade50, // Thêm nền xám nhạt để khối trắng nổi bật
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: Column(
                children: [
                  _buildInfoRow(Icons.schedule, 'Lịch sinh hoạt', club.meetingTime ?? 'Đang cập nhật'),
                  const Padding(
                    padding: EdgeInsets.symmetric(vertical: 14),
                    child: Divider(height: 1, color: AppColors.border),
                  ),
                  _buildInfoRow(Icons.location_on_outlined, 'Địa điểm', club.location ?? 'Đang cập nhật'),
                  const Padding(
                    padding: EdgeInsets.symmetric(vertical: 14),
                    child: Divider(height: 1, color: AppColors.border),
                  ),
                  _buildInfoRow(Icons.person_outline, 'Liên hệ', club.contact ?? 'Đang cập nhật'),
                  const Padding(
                    padding: EdgeInsets.symmetric(vertical: 14),
                    child: Divider(height: 1, color: AppColors.border),
                  ),
                  _buildInfoRow(Icons.people_alt_outlined, 'Thành viên', '${club.memberCount} người'),
                ],
              ),
            ),
  ...
  Widget _buildInfoRow(IconData icon, String title, String value) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.center, // Căn giữa icon với nội dung 2 dòng text
      children: [
        Icon(icon, color: AppColors.primary, size: 24), // Tăng size lên xíu nhìn sẽ đẹp hơn khi căn giữa
