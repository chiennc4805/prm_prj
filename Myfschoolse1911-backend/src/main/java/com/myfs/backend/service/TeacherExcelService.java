...
public class TeacherExcelService {
  ...
  public TeacherExcelService(AppUserDao users) {
    this.users = users;
  }
  ...
  public PreviewResult preview(MultipartFile file) {
    ...
    try (Workbook workbook = WorkbookFactory.create(file.getInputStream())) {
      ...
      if (sheet.getLastRowNum() > MAX_ROWS) throw bad(
        "File chỉ được chứa tối đa " + MAX_ROWS + " giáo viên"
      );
      DataFormatter formatter = new DataFormatter();
      for (int index = 1; index <= sheet.getLastRowNum(); index++) {
        Row row = sheet.getRow(index);
        String name = cell(row, 0, formatter),
          phone = cell(row, 1, formatter),
          email = cell(row, 2, formatter);
      ...
      throw bad(
        "Không thể đọc file Excel. Hãy sử dụng file .xlsx đúng định dạng"
      );
    }
  ...
  public ImportResult importTeachers(ImportRequest request) {
    if (
      ...
      request.teachers().isEmpty()
    ) throw bad("Không có giáo viên hợp lệ để nhập");
    if (request.teachers().size() > MAX_ROWS) throw bad(
      "Chỉ được nhập tối đa " + MAX_ROWS + " giáo viên"
    );
    ...
    for (int i = 0; i < request.teachers().size(); i++) {
      ...
      List<String> rowErrors = validate(
        clean(item.fullName()),
        clean(item.phone()),
        clean(item.email()),
        phones,
      ...
      if (!rowErrors.isEmpty()) errors.add(
        "Dòng " + (i + 1) + ": " + String.join(", ", rowErrors)
      );
    }
    if (!errors.isEmpty()) return new ImportResult(
      0,
    ...
    for (ImportItem item : request.teachers()) {
      ...
      teacher.setFullName(clean(item.fullName()));
      teacher.setPhone(clean(item.phone()));
      teacher.setEmail(emptyToNull(item.email()));
  ...
  private List<String> validate(
    ...
    if (phone.isBlank()) errors.add("Số điện thoại không được để trống");
      ...
      if (!phonesInFile.add(phone)) errors.add(
        "Số điện thoại bị trùng trong file"
      );
      if (users.findByPhone(phone).isPresent()) errors.add(
        "Số điện thoại đã tồn tại"
      );
    }
    if (!email.isBlank()) {
      if (!EMAIL.matcher(email).matches()) errors.add(
        "Email không đúng định dạng"
      );
      else {
        if (!emailsInFile.add(email.toLowerCase())) errors.add(
          "Email bị trùng trong file"
        );
        if (users.findByEmail(email).isPresent()) errors.add(
          "Email đã tồn tại"
        );
      }
    }
  ...
  private void validateFile(MultipartFile file) {
    ...
    String name = Optional.ofNullable(file.getOriginalFilename())
      .orElse("")
      .toLowerCase();
    if (!name.endsWith(".xlsx")) throw bad(
      "Chỉ hỗ trợ file Excel định dạng .xlsx"
    );
    if (file.getSize() > 5 * 1024 * 1024) throw bad(
  ...
  private String cell(Row row, int index, DataFormatter formatter) {
    return row == null
      ? ""
      : clean(formatter.formatCellValue(row.getCell(index)));
  }
