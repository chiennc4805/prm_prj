...
class _GradeDetailScreen extends StatelessWidget {
  ...
  Widget _gradeItemsTable(List<GradeItem> items) => Card(
    clipBehavior: Clip.antiAlias,
    child: Table(
      columnWidths: const {
      ...
      children: [
        const TableRow(
          ...
          children: [
            _GradeCell('Đầu điểm', header: true),
            _GradeCell('Hệ số', header: true, centered: true),
            _GradeCell('Điểm số', header: true, centered: true),
          ],
        ),
        ...items.asMap().entries.map(
          (entry) => TableRow(
            decoration: BoxDecoration(
              color: entry.key.isEven ? Colors.white : const Color(0xFFFAFAFA),
            ),
            children: [
              _GradeCell(entry.value.name),
              _GradeCell(
                entry.value.weight.toStringAsFixed(
                  entry.value.weight % 1 == 0 ? 0 : 1,
                ),
                centered: true,
              ),
              _GradeCell(
...
class _GradeCell extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Text(
      ...
      style: TextStyle(
        color: score ? AppColors.primary : AppColors.ink,
        fontSize: header ? 12 : 13,
        fontWeight: header || score ? FontWeight.w800 : FontWeight.w600,
      ),
    ),
  );
}
