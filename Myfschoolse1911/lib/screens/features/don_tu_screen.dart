...
class _DonTuScreenState extends State<DonTuScreen> {
  ...
  Future<List<LeaveRequest>> load() =>
      student == null ? Future.value([]) : LeaveService.byStudent(student!.id);
  void reload() => setState(() => future = load());
  Future<void> openForm() async {
    if (!Session.instance.isParent || student == null) return;
    final ok = await Navigator.push<bool>(
      context,
      MaterialPageRoute(builder: (_) => _LeaveForm(student: student!)),
    );
    if (ok == true) reload();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
              ...
              builder: (context, snapshot) {
                ...
                if (rows.isEmpty)
                  return EmptyView(
                    ...
                    message: 'Chưa có đơn xin nghỉ học.',
      ...
      floatingActionButton: Session.instance.isParent && student != null
  ...
  Widget _card(LeaveRequest r) {
    ...
    final date = r.fromDate == r.toDate
        ? r.fromDate
        : '${r.fromDate} → ${r.toDate}';
    return Card(
          ...
          children: [
            Row(
              children: [
                ...
                Chip(
                  label: Text(LeaveRequest.label(r.status)),
...
class _LeaveFormState extends State<_LeaveForm> {
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: const Text('Tạo đơn xin nghỉ học')),
    body: Form(
      key: formKey,
      child: ListView(
        ...
        children: [
          ...
          FilledButton(
            ...
            child: Text(saving ? 'Đang gửi...' : 'Gửi đến GVCN'),
          ),
        ],
      ),
    ),
