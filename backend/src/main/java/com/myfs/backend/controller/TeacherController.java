...
public class TeacherController {
  ...
  public Notification notify(
    ...
    boolean allowed =
      assignments.existsByTeacherIdAndClassId(teacherId, classId) ||
      classes
        .findById(classId)
        .map(c -> teacherId.equals(c.getHomeroomTeacherId()))
        .orElse(false);
    if (!allowed) throw new ResponseStatusException(
  ...
  @PutMapping("/homeroom/{classId}/leaves/{leaveId}")
  public LeaveRequest review(
    ...
    access.requireHomeroom(classId, teacherId);
    if (
      !Set.of("APPROVED", "REJECTED").contains(body.status())
    ) throw new ResponseStatusException(
      HttpStatus.BAD_REQUEST,
      "Trạng thái phải là APPROVED hoặc REJECTED"
    );
    LeaveRequest r = leaves
      .findById(leaveId)
      .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND));
    Student s = access.requireStudentInClass(r.getStudentId(), classId);
    r.setStatus(body.status());
    r.setRejectionReason(
      "REJECTED".equals(body.status()) ? body.rejectionReason() : null
    );
    r.setReviewedById(teacherId);
  ...
  private void validateItems(List<GradeItem> items) {
    if (items == null || items.isEmpty()) throw new ResponseStatusException(
      HttpStatus.BAD_REQUEST,
    ...
    for (GradeItem i : items)
      if (
        ...
        HttpStatus.BAD_REQUEST,
        "Đầu điểm, giá trị và trọng số không hợp lệ"
      );
  }
